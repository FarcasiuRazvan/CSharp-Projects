...
namespace Internship
{
    public partial class CustomGridForm : Form
    {
        ...
        // This function will initialise the data table(dt) and set it as the data source for the data grid view.
        private void InitialiseDataTable()
        {
            ...
            {
                MessageBox.Show("Height or width are not integer values, please input other values !!!", "Wrong Input", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
            else
            {
                dt = new DataTable();
        ...
        // This function will get the initial pins from the grid and will check if there are empty cells.
        private void getInitialPinsFromGrid() {
            for (int i = 0; i < dt.Rows.Count; i++)
                for (int j = 0; j < dt.Columns.Count; j++)
                {
                    if (dt.Rows[i][j.ToString()].ToString() == "") throw new Exception("There are empty cells in the grid, please fill them up !!!");
                    if (dt.Rows[i][j.ToString()].ToString() != "0") pinsList.Add(new Tuple<int, int>(i, j));
                }
        }
        ...
        private void btnRoute_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                while (this.IsRoutingDone() == false)
                {
                    ...
                    {
                        // ROUTING until current route can be finished.
                        this.routing(t[0], t[1], t[0].Item1, t[0].Item2, "Start");
                        // If foundRoute is false then eliminate the closest route and do this route again.
                        if (foundRoute == false)
                        {
                            contRerouting += 1;
                    ...
                    foundRoute = false;
                    if (t[0].Item1 != -1 && t[0].Item2 != -1) addedPinsList.Add(t[0]);
                    if (t[1].Item1 != -1 && t[1].Item2 != -1 && addedPinsList.Contains(t[1]) == false) addedPinsList.Add(t[1]);
                }
            }
            catch (Exception exception)
            {
                MessageBox.Show(exception.Message, "Wrong Input", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
